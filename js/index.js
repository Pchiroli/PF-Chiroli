const FORM_RESERVA = document.createElement('form');
  ...
  fetch('./json/cabanias.json')
    .then(response => {
      if (!response.ok) {
        throw new Error('Error al cargar el JSON de cabañas');
      }
      return response.json();
    })
    .then(cabanias => {
      window.CABANIAS = cabanias;
    })
    .catch(error => {
...
if (!CABANIA_DISPONIBLE) {
  Swal.fire({
    icon: "error",
    title: "Oops...",
    text: "Lo sentimos, no hay cabañas disponibles para la cantidad de personas seleccionada.",
  })
  return;
}
